...
sst <- tibble(dir = dirs) %>% 
  ...
  select(ssp, model) %>% 
  distinct() %>% 
  expand_grid(decade = c("20")) %>% #, "202", "203", "204", "205")) %>% 
  head(1) %>% 
...
extracted <- sst %>% 
  mutate(sst = map(.x = s,
                   .f = ~exact_extract(x = .x,
                                       ...
                                       progress = T))) %>% 
  select(ssp, model, decade, files, sst)
...
## EXPORT ######################################################################
my_write <- function(ssp, model, decade, sst) {
  
  main_dir <- here("data", "processed", "CMIP_SST_projections", ssp)
  filename <- paste0(model, "_", decade, ".rds")
...
# X ----------------------------------------------------------------------------
extracted_long %$%
  pwalk(list(ssp, model, decade, sst),
        my_write)
